...
int main() {
	...
	Rect rect5, rect6, rect7, rect8; //To set roi of each seats
	...
	int bs1 = 0, bs2 = 0, bs3 = 0, bs4 = 0;//For check the Background Subtraction of each seats
	...
	rect5 = Rect(30, 300, 120, 160); //왼쪽 아래	
	rect6 = Rect(280, 290, 120, 160);//오른쪽 아래
	...
	if (whatdoyouwant == 1) {
		while (1) {
			...
			if (count > 120)
			{
				putText(fullFrame, "Don't just leave your luggage", Point(20, 150), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
			}
			cout << "count: " << count << endl << endl;
			putText(fullFrame, format("Remaining seats: %d", (4 - sit)), Point(50, 50), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 0), 2);
	...
	//If half
		...
		while (1) {
			...
			Broi1 = halfMask(rect5);
			Broi2 = halfMask(rect6);
			...
			if (count > 120)
			{
				putText(halfFrame, "Don't just leave your luggage", Point(20, 150), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
			}
			cout << "count: " << count << endl << endl;
			putText(halfFrame, format("Remaining seats: %d", (4 - sit)), Point(50, 50), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 0), 2);
	...
	//If empty
		...
		while (1) {
			...
			if (count > 120)
			{
				putText(emptyFrame, "Don't just leave your luggage", Point(20, 150), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
			}
			cout << "count: " << count << endl << endl;
			putText(emptyFrame, format("Remaining seats: %d", (4 - sit)), Point(50, 50), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 0), 2);
...
int isBacksub(Mat roi) {
	...
	avg = (float)count / (roi.rows * roi.cols);
	if (avg <= 0.2) return 0;
	else return 1;
}
